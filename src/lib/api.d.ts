...
export interface paths {
    ...
    "/v1/send": {
        ...
        put?: never;
        /**
         * Answer one message the corpus holds, and file the answer into the corpus.
         ...
         *
         *     Reply-only, and there is deliberately no recipient in the request. The message answered is named by its corpus ext id, and who the reply goes to and what it is called come from that message's own headers. A surface that can only answer mail the corpus already holds has no arbitrary recipient — no address to type, no way for a page behind a loopback bind with no authentication to become an outbound channel — which is what makes this a reading tool that can reply rather than a mail-sending endpoint.
         *
         *     Two steps, and the first sends nothing: without `confirm` the reply is prepared and the answer is the plan — the recipient the mailbox will use, the subject, and the whole body including the quote of the message being answered, which this server composes. That is a preview of the message rather than of a draft. With `confirm` the same prepare runs and is executed, and the answer carries the id of the message that went out.
         *
...
export interface components {
    schemas: {
        ...
        /** @description A reply: which message is being answered, what the reader has to say, and whether this call is the preview or the send. There is no recipient field — the address is the one the message being answered arrived from. */
        SendRequest: {
            /**
        ...
        /** @description The reply as the mailbox has it, and whether this call sent it. The same three fields are answered by a preview and by a send, so what went out can be checked against the plan a reader was shown rather than trusted. */
        SendResponse: {
            /** @description The message this reply answers, echoed back so a client does not have to infer it from the request it sent. */
            entry: string;
            /**
             * @description The recipient the mailbox will use, which is the address the message being answered arrived from — as that message's own header states it, names included, not a name the corpus resolved. Nothing here lets a caller choose it.
...
export interface operations {
    ...
    sendReply: {
        ...
        responses: {
            /** @description The reply as the mailbox has it: the recipient and subject from the headers of the message being answered, and the body with the quote under the reader's own words — plus whether this call sent it. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
