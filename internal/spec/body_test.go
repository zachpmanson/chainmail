...
// renderText is textToHTML for a body with no boilerplate detected, which is
// every case here but the fold tests below. Those call textToHTML directly with
// the fold they are about.
func renderText(body string, st bodyStyle) string {
	s, _ := textToHTML(body, st, boiler.Fold{})
	return s
}
...
func TestAngleBracketAddressSurvivesAsVisibleText(t *testing.T) {
	...
	got := renderText("Ping <ada@loomworks.example> about the cutover.", mailBody)
...
func TestMarkupInTheTextCannotBecomeMarkupOnThePage(t *testing.T) {
	got := renderText(`<script>alert(1)</script> and <b onmouseover="x">bold</b>`, mailBody)
...
func TestUndecodedEntityIsShownAsTheCharactersItIs(t *testing.T) {
	...
	got := renderText("Loom &amp; Fjord, 10&nbsp;kW", mailBody)
...
func TestReflowLeavesColumnsAndIndentationAlone(t *testing.T) {
	...
	indented := "    if unit == \"$/kW\" {\n        return v / days\n    }"
	got = renderText(indented, mailBody)
...
func TestQuotedHistoryIsPeeledFromAMailBodyButAnInlineQuoteIsKept(t *testing.T) {
	...
	inline := "Root cause from the pull request:\n" +
		...
		"\n" +
		"Fixed on the branch."
	got = renderText(inline, mailBody)
...
func TestDeeperQuotesNest(t *testing.T) {
	got := renderText("Quoting a quote:\n\n> she said\n>> he said\n", mailBody)
...
func TestAQuotedEntryKeepsItsOwnTextEvenWhenItLooksLikeAHeaderBlock(t *testing.T) {
	...
	if got := renderText(body, quotedMail); !strings.Contains(got, "please action.") {
		t.Errorf("body = %q, want the text kept", got)
	}
}

func TestOnlyHTTPSchemesAreLinkified(t *testing.T) {
	got := renderText("See https://loomworks.example/plan and javascript:alert(1) now", mailBody)
...
func TestLinkifyingCannotBreakOutOfTheHref(t *testing.T) {
	...
	got := renderText(`https://loomworks.example/a"onmouseover="alert(1)`, mailBody)
...
func TestTheRFCDelimiterFoldsAndDoesNotDelete(t *testing.T) {
	...
	// The difference between folding and deleting, asserted rather than assumed:
	// the phone number is off screen and still in the document, so find-in-page
	// and grep over the exported file both reach it.
	if !strings.Contains(got, "+61 400 000 000") {
		t.Errorf("body = %q, want the signature folded, not removed", got)
	}
	if strings.Contains(got, "[signature trimmed]") {
		t.Errorf("body = %q, want no trim notice: nothing was trimmed", got)
	}
}

func TestEmptyTextProducesAnEmptyBodyNotEmptyMarkup(t *testing.T) {
	for _, in := range []string{"", "   ", "\n\n \n"} {
		if got := renderText(in, mailBody); got != "" {
			t.Errorf("renderText(%q) = %q, want the empty string", in, got)
		}
	}
	...
	if got := renderText(quoteOnly, mailBody); got != "" {
